...
public class TroopDescriptionPopup : PopupBase
{
    ...
    [SerializeField] private TMP_Text nextSpeedText;
    ...
    //     troopLevel = GameManager.GetInstance().GetTroopLevel(troopDataSO.TroopId);
    ...
    //         nextSpeedText.gameObject.SetActive(false);
    ...
    // {
    //     TroopUpgradeCost upgradeCost = troopDataSO.GetUpgradeCost(level);

    //     int currentFragments = GameManager.GetInstance().GetCurrentTroopFragmentsAmount(troopDataSO.TroopId);

    //     fragmentsSlider.maxValue = upgradeCost.fragmentsRequired;
    ...
    //     if (currentFragments >= upgradeCost.fragmentsRequired)
    //     {
    //         upgradeCostFragmentsString = $" <color=white>{currentFragments}</color> / {upgradeCost.fragmentsRequired}";
    //         sliderFillImage.sprite = progressEndSprite;
    //     }
    //     else
    //     {
    //         upgradeCostFragmentsString = $" <color=red>{currentFragments}</color> / {upgradeCost.fragmentsRequired}";
    ...
    //     string upgradeCostCoinsString = "";

    //     if (currentCoins >= upgradeCost.coinsRequired)
    //     {
    //         upgradeCostCoinsString = $" <color=white>{upgradeCost.coinsRequired}</color>";
    //     }
    //     else
    //     {
    //         upgradeCostCoinsString = $" <color=red>{upgradeCost.coinsRequired}</color>";
    //     }

    //     currencyTextCoins.text = upgradeCostCoinsString;

    //     if (currentFragments >= upgradeCost.fragmentsRequired && currentCoins >= upgradeCost.coinsRequired)
    //     {
    ...
    // {
    //     TroopUpgradeCost upgradeCost = troopDataSO.GetUpgradeCost(troopLevel);
    //     int currentFragments = GameManager.GetInstance().GetCurrentTroopFragmentsAmount(troopDataSO.TroopId);
    //     int currentCoins = GameManager.GetInstance().GetCurrentCoinAmount();
    ...
    //         GameManager.GetInstance().SetTroopLevel(troopDataSO.TroopId, troopLevel + 1);
    //         troopLevel++;
    //         GameManager.GetInstance().AddTroopFragments(troopDataSO.TroopId, -upgradeCost.fragmentsRequired);
    //         GameManager.GetInstance().AddCoins(-upgradeCost.coinsRequired);
    //         SetStats(troopDataSO, troopLevel);
    //         SetUpgradeCurrency(troopDataSO, troopLevel);
    ...
    //         ToastMessageManager.GetInstance().ShowToastMessage("Not enough fragments or coins to upgrade", 2f);
    //         return;
    //     }
    // }
    ...
    //     currentSpeedText.transform.DOPunchScale(Vector2.one * 0.25f, 0.5f, 1, 0.5f);
