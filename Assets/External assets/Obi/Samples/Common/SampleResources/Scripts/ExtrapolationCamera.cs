...
public class ExtrapolationCamera : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (target != null)
        {
			// Get poleTransform delta since the last physics update:
			Vector3 positionDelta = target.position - lastPosition;
            positionDelta.y = 0;

            // extrapolate poleTransform using velocity (the division/multiplication by Time.deltaTime simplify out)
    ...
    void LateUpdate()
    {
        if (target != null)
        {
            // get vector from the camera to the extrapolated poleTransform:
            Vector3 toTarget = extrapolatedPos - transform.position;

            // rotate the camera towards the extrapolated poleTransform:
            ...
            // move the camera towards the extrapolated poleTransform, keeping some distance to it:
